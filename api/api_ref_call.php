...
function get_oauth() {
    ...
    if(is_bool($response) && $response == true){
        ...
        try {
            ...
            if(count($the_rec) > 0){
                ...
                error_log('sandbox user not found');
                exit(json_encode($arr));
            }

        } catch (Exception $Exception) {
            error_log('sandbox user not found exception');
    ...
    if(!isset($oauth_user)){
        try {
            ...
            error_log('oauth token missing or invalid exception');
            exit(json_encode(array("result"=>"ERROR OAuth token missing or invalid.")));
        }
    }
    ...
    if(isset($oauth_user)){
        ...
        if(count($ret)>0) {
            ...
            error_log('institution not found');
