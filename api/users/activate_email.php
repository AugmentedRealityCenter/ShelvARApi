<?php
	...
	if(!count($err)) {
		...
		if($result[0]['email_verified'] == 1) {
			...
			$is_admin = 0;
			$permissions = 0;
			//Sandbox users get approved for everything except admin
			if($inst_id == "sandbox"){
			  $permissions = 1;
			}
			...
			$db = new database();
			$db->query = "UPDATE users SET pending_email = ?, email = ?, email_verified = ?, is_admin = ?, can_submit_inv = ?, can_read_inv = ?, can_shelf_read = ? WHERE user_id = ?";
			$db->params = array($pending_email, $email, $email_verified, $is_admin, $permissions, $permissions, $permissions, $user_id);
			$db->type = 'ssiiiiis';
